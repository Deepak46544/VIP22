...
def main_menu_keyboard():
    return ReplyKeyboardMarkup([ 
        [KeyboardButton("/Restart ▶️"), KeyboardButton("/Stop ⏸️")],
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id not in users:
        await update.message.reply_text("❌ You don't have an active subscription. Please contact the admin for assistance.Buy Form @raj14754", reply_markup=main_menu_keyboard())
    else:
        expiration_date = users[user_id]
        await update.message.reply_text(f"👋 Welcome {username}!\n Your subscription is active until {expiration_date}.\n This Tool is provided by @raj14754", reply_markup=main_menu_keyboard())
...
async def stop_attack(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        await update.message.reply_text("✅ Attack stopped.@raj14754", reply_markup=main_menu_keyboard())
    except Exception as e:
...
async def pause_attack(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        await update.message.reply_text(f"❌ Error stop attack: {str(e)}")
...
async def resume_attack(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        await update.message.reply_text("✅ Attack restart.@raj14754")
    except Exception as e:
        await update.message.reply_text(f"❌ Error restart attack: {str(e)}")

async def view_attacks(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    user_id = str(update.message.from_user.id)
    if user_id not in user_processes:
        await update.message.reply_text("📊 No ongoing attacks.@raj14754")
        return
...
async def attack_remove(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id in user_processes and user_processes[user_id]["id"] == attack_id:
        ...
        try:
            process.terminate()
            del user_processes[user_id]
            await update.message.reply_text(f"✅ Attack with ID {attack_id} has been stopped and removed. @raj14754")
            attack_found = True
        except Exception as e:
...
async def check_bgmi_traffic(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text("📈 Checking BGMI traffic...\nThe current traffic status is normal. No issues detected.@raj14754")
    ...
    if user_id in ADMIN_IDS:
        ...
        if len(command) == 2:
            try:
                ...
                response = "Please specify a valid number and unit of time (hours/days).@raj14754"
        else:
            response = "Usage: /genkey <amount> <hours/days>"
    else:
        response = "ONLY OWNER CAN USE💀OWNER @raj14754"
...
async def redeem(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(command) == 1:
        ...
        if key in keys:
            ...
            response = "Invalid or expired key buy from @raj14754."
    else:
...
async def allusers(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id in ADMIN_IDS:
        ...
        response = "ONLY OWNER CAN USE.@raj14754"
    await update.message.reply_text(response)
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text("ℹ️ Help Menu:\n"
                                      ...
                                      "/stop - Pause an ongoing attack\n"
                                      "/restat - Resume a paused attack\n"
...
if __name__ == '__main__':
    ...
    app.add_handler(CommandHandler("stop", pause_attack))
    app.add_handler(CommandHandler("restart", resume_attack))
